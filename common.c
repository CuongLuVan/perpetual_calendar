...
long getJulius(long intNgay, long intThang, long intNam)
{
	...
	y = intNam + 4800 - a;
	m = intThang + 12 * a - 3;
	jd = intNgay + ((153 * m + 2) / 5) + 365 * y + (y / 4)  - (y / 100) + (y / 400) - 32045;
	if (jd < 2299161)
	{
	jd = intNgay + ((153 * m + 2) / 5) + 365 * y + (y / 4)  - 32083;
	}
	return jd;
}
...
long getLunarMonthll(long intNam){
	...
	nm = getNewMoonDay((long)k);
	// sun longitude at local midnight
	sunLong = getSunLongitude((long)nm);
	if (sunLong >= 9)
	{
	nm = getNewMoonDay((long)k - 1);
	}
return (long)nm;
}
...
long getLeapMonthOffset(double a11){
	...
	i = 1;
	arc = getSunLongitude((long)getNewMoonDay((long)(k + i)));
	do
	{
	last = arc;
	i++;
...
int convertSolar2Lunar(int intNgay, int intThang, int intNam){
	...
	k = (long)((dayNumber - 2415021.076998695) / 29.530588853);
...
//return printf("\n Ngay am: %d/%d/%d",Ngay1, Thang1, Nam1);
//    label1.Text=Ngay1.ToString() +"  //" +Thang1.ToString()+ "//" +Nam1.ToString();
	datasave[STATEDAYLUNAR]=(int)(lunarDay);
	datasave[STATEMOUNTLUNAR]=(int)(lunarMonth);
	datasave[STATEYEARLUNAR]=(int)(lunarYear);
	return 0;
}
